...
imajo v $\R^3$ obliko stožca. Naj bo stožnica $\mathcal{S}$ s pripradajočo matriko $M$ neizrojena. Če jo zožimo na afini del $\Pi: z=1$, lahko iz prvega minorja matrike $M$ takoj preberemo, za katero vrsto neizrojene stožnice gre (za algebraično razlago gl.\ \cite{sivic2020}). Naj bo $A_M = a_{11}a_{22} - a_{12}^2$. Potem velja:
\begin{itemize}
...
\begin{opomba}
    ...
    $$p_A =\{ [x : y : z] \in P(\R^3) \; | \; [x, y, z] \cdot M \cdot a = 0 \}, \; a \in A.$$
...
Vzemimo kubično enačbo oblike $ x^3 + bx^2 + cx + d = 0, d \neq 0$. Vzamemo naslednji stožnici, ki sta v zožitvi na afini del paraboli:
\begin{itemize}
...
Enačbi parabol razširimo v projektivno ravnino in iz njiju zapišemo njuni matriki
$$ M_1 =
    \begin{bmatrix}
        0 & 0 & 2d\\
        0 & 1 & c\\
