...
Kot primer tega Hull v~\cite[str.\ 38]{hull2020} navaja Lillovo konstrukcijo rešitve kvadratne enačbe $x^2 + bx + c = 0$ preko krožnice. Skozi točke $A = (0,1), \; H = (-b, 0)$ in $B = (-b, c)$ narišemo krožnico s premerom $AB$ in njeno središče označimo s $C$ (slika~\ref{fig:par_primer_hull_lill} (a)). Presečišči krožnice z abscisno osjo (če obstajata) označimo s točkama $M$ in $N$. Njuni abscisi sta nato rešitvi enačbe (lahek dokaz, zakaj konstrukcija deluje, je prepuščen bralcu). Konstrukcijo brez težav prevedemo v origami konstrukcijo -- točki $M$ in $N$ sta sliki točke $A$ preko pregiba, ki poteka skozi središče daljice $AB$ (slika~\ref{fig:par_primer_hull_lill} (b)).
...
Enačba~\eqref{eq:spl_kv_en} nam ob teh predpostavkah torej poda pokončno parabolo $y = x^2 + bx + c$ z vodoravno premico vodnico in dvema ničlama, ki sta rešitvi naše enačbe. Iščemo abscisi presečišč parabole z abscisno osjo.
...
Naj bo $t$ ena izmed rešitev enačbe~\eqref{eq:spl_kv_en}. Na premici $a$ z $A'$ označimo točko z absciso $t$. Poiščimo enačbo pregiba, ki gorišče $A$ položi v točko $A'$. Ta pregib bo tangenten na parabolo ravno v njeni ničli, njegovo presečišče z osjo parabole $ x = -b/2 $ pa nam bo določilo točko $B$.
...
Če pod nobenim kotom $\theta$ ne moremo zadeti želve, je polinom $p$ brez realnih ničel. Zato je na mestu vprašanje, kako določiti kot $\theta$. Za polinom tretje stopnje je Belocheva preko svojega pregiba našla zelo preprosto rešitev, ki si jo bomo sedaj pogledali.

\subsubsection*{Belochin kvadrat}

Imejmo dani točki $A$ in $B$ ter premici $r$ in $s$. Z origamijem konstruirajmo kvadrat $WXYZ$, kjer oglišče $X$ leži na premici $r$, njegovo sosednje oglišče $Y$ pa na premici $s$. Velja še, da točka $A$ leži na stranici $WX$ (ali njeni nosilki), točka $B$ pa na stranici $ZY$ (ali njeni nosilki). Kvadrat po avtorici njegove konstrukcije imenujemo \emph{Belochin kvadrat} (slika~\ref{fig:beloch_kvadrat}).
...
\begin{enumerate}
    \item Naj bosta dani parabola z znanima goriščem in premico vodnico ter krožnica z znanima središčem in polmerom. Teh dveh stožnic ni potrebno risati, ker potrebujemo le omenjene podatke.
...
Poglejmo si model realne projektivne ravnine, ki ga označimo s $P(\R^3)$. Točke našega modela so predstavljene z evklidskimi premicami v $\R^3$ skozi koordinatno izhodišče (imenujmo jih \emph{projektivne točke}), premice pa z evklidskimi ravninami v $\R^3$, ki vsebujejo izhodišče (imenujmo jih \emph{projektivne premice}).

Naj bosta v $\R^3$ dani točka $P = (x_0, y_0, z_0)$, ki ni koordinatno izhodišče, in premica $p$, ki poteka skozi koordinatno izhodišče in vsebuje točko $P$. Premica $p$ predstavlja projektivno točko v $P(\R^3)$, ki jo označimo s t.~i.\ \emph{homogenimi} koordinatami $[x_0 : y_0 : z_0]$. Ker za poljuben neničeln $\lambda \in \R$ točka s koordinatami $(\lambda x_0, \lambda y_0, \lambda z_0)$ določa isto premico $p$ kot točka $P$ in s tem isto projektivno točko, lahko pišemo $[\lambda x_0 : \lambda y_0 : \lambda z_0] = [x_0 : y_0 : z_0]$.

Poljubna ravnina v $\R^3$ skozi koordinatno izhodišče ima enačbo $ax+by+cz=0$, kjer so $a, b, c \in \R$, ne vsi enaki $0$. Projektivna premica, ki jo ta ravnina predstavlja, je tako množica projektivnih točk $\{ [x:y:z] \; | \; ax+by+cz = 0\}$.

Realno ravnino $\R^2$ lahko vložimo v projektivno ravnino $P(\R^3)$. Ponavadi to storimo tako, da jo postavimo na nivo $z=1$ (slika~\ref{fig:vlozitev_proj_ravnina}):
\begin{itemize}
    \item Točka $(x_0, y_0)$ v $\R^2$ določa točko $P = (x_0, y_0, 1)$ na tej ravnini, ki v $\R^3$ določa enolično premico $p$ skozi $P$ in izhodišče, kar je v $P(\R^3)$ projektivna točka $[x_0 : y_0 : 1]$.
    \item Premica $ax + by + c = 0$ v $\R^2$ določa premico v $\R^3$ z enako enačbo, ki leži na ravnini $z = 1$, to pa v $\R^3$ določa enolično ravnino, ki vsebuje to premico in koordinatno izhodišče, kar je v $P(\R^3)$ projektivna premica $\{ [x:y:z] \; | \; ax+by+cz = 0\}$.
\end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/projektivna_ravnina.jpg}
    \caption[Vložitev realne ravnine v projektivno]{Vložitev realne ravnine $\R^2$ v projektivno ravnino $P(\R^3$).}
    \label{fig:vlozitev_proj_ravnina}
\end{figure}

Evklidske premice skozi koordinatno izhodišče, ki ne sekajo ravnine $z = 1$ (torej ležijo na ravnini $z=0$), predstavljajo projektivne točke, ki jih imenujemo \emph{točke v neskončnosti}. Te točke imajo tretjo koordinato ničelno. Sama ravnina $z = 0$ predstavlja t.~i.\ \emph{premico v neskončnosti}, ki jo označimo s $p_\infty$.

% Ključna lastnost projektivne ravnine je, da se v njej sekata \emph{vsaki dve} poljubni projektivni premici (v ekvlidski ravnini to ne velja, saj obstajajo vzporedne premice). To hitro vidimo -- vsaki dve ravnini v $\R^3$, ki vsebujeta koordinatno izhodišče (torej vsaki dve projektivni premici), sekata v neki evklidski premici, ki vsebuje izhodišče (torej v projektivni točki, ki je predstavljena s premico v presečišču teh dveh ravnin). Ko je ena izmed projektivnih premic premica v neskončnosti, v preseku dobimo \emph{točko v neskončnosti}. Tako si lahko projektivno ravnino lahko predstavljamo kot običajno ravnino, ki ima dodano premico v neskončnosti ($z = 0$).

% Tako veljajo naslednji aksiomi projektivne ravnine:
% \begin{itemize}
%     \item [P1:] Vsak par različnih točk leži na natanko eni premici.
%     \item [P2:] Vsak par različnih premic se seka v natanko eni točki.
%     \item [P3:] Obstajajo 4 točke, od katerih nobena trojica ni kolinearna.
% \end{itemize}

% V projektivni ravnini velja \emph{načelo dualnosti} -- vsaka značilnost, ki velja za premice, velja tudi za točke in obratno. Na splošno lahko v vsakem izreku ravninske projektivne geometrije zamenjamo besedi ``točke'' in ``premice'' med sabo, pa bo izrek še vedno veljal. Če malo preoblikujemo aksioma P1 in P2, vidimo, da sta drug drugemu dualna izjava:
% \begin{itemize}
...
Med projektivnimi točkami in premicami obstaja bijekcija. To je \emph{dualna preslikava}, ki objektu v projektivni ravnini priredi njegov ortogonalni komplement. Iz tega takoj sledi, da je dualna preslikava sama sebi inverz.

Kot primer dualna preslikava projektivni točki, tj.\ evklidski premici z nekim smernim vektorjem, priredi projektivno premico, tj.\ evklidsko ravnino, ki ima ta smerni vektor za normalo. Natančneje je to prikazano v tabeli~\ref{tab:dualna_preslikava}, kjer je dodana tudi povezava med realno in projektivno ravnino. Puščice lahko obrnemo tudi v nasprotno smer in iz premice v realni ravnini pripadajočo projektivno premico slikamo v projektivno točko, le-to pa v točko v realni ravnini.
...
\begin{table}[h!]
    ...
    & $\R^2$ & & $P(\R^3)$ \\
    \hline \\
    točka & $(a,b)$ & $\xrightarrow{\text{vložitev v } P(\R^3)}$ & $[a : b : 1] $ \\
    & & & \\
    & &  & \textcolor{red}{$\downarrow$} \\
    & & & \\
    premica & $ ax + by + 1 = 0$ & $\xleftarrow{\text{zožitev na } z = 1} $ & $\{ [x:y:z] \; | \; ax+by+z = 0 \} $
    \end{tabular}
    \end{center}
    \caption{Dualna preslikava projektivne točke v projektivno premico.}
    \label{tab:dualna_preslikava}
\end{table}

Sliki objekta, ki ga preslikamo z dualno preslikavo, rečemo \emph{dual}. V nadaljevanju bomo obravnavali duale stožnic, zato si poglejmo sledeči primer.

\textbf{Primer:} Vzemimo parabolo z enačbo $y = x^2$ v realni ravnini. Naj bo $A = (a, a^2)$ točka na tej paraboli, ki v projektivni ravnini določa projektivno točko $[a: a^2: 1]$. Z dualno preslikavo se le-ta preslika v projektivno premico $\{ [x:y:z] \; | \; ax+a^2y+z = 0 \} $, kar je v afini ravnini evklidska premica $ax + a^2y + 1 = 0$ (slika~\ref{fig:dual3D}).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{images/dualna_preslikava_3D2.png}
    \caption[Vizualizacija dualne preslikave]{Točka $[a:a^2:1]$ se preslika v premico -- evklidsko ravnino $ax+a^2y+z=0$.}
    \label{fig:dual3D}
\end{figure}

Zožitve projektivne premice, ki jo dobimo z dualno preslikavo projektivnih točk s homogenimi koordinatami $[a:a^2:1]$, na ravnino $z=1$ sestavljajo družino premic s parametrizacijo $F(a,x,y) = ax+a^2y+1=0$, kjer je $a \in \R$ parameter. Iz začetka poglavja~\ref{pogl:stoznice} že vemo, da so te premice tangente na ovojnico te družine, katere enačbo že znamo izračunati. Iz formule $\frac{\partial}{\partial a} F(a, x, y) = 0$ dobimo zvezo $a = -x/2y$, ko to vstavimo v formulo $F(a, x, y) = 0$, pa $4y = x^2$.

V naslednjem razdelku si bomo pogledali, kako stožnice in njihove duale predstavimo v projektivni ravnini. Videli bomo, da smo s tem primerom v resnici preslikali parabolo z enačbo $yz = x^2$ v njeno dualno parabolo z enačbo $4yz = x^2$, pri čemer so nam zožitve slik projektivnih točk s prve parabole podale afine tangente na zožitev dualne parabole na afino ravnino. Naš primer je bil lažje izračunljiv, saj je bila dana parabola v afini ravnini funkcija, v splošnem pa stožnice niso funkcije in točk na njih ne moremo opisati kot $(a, f(a))$, kjer je $f$ funkcija.
...
\end{definicija}
\begin{opomba}
    Med enačbama stožnic v obeh ravninah obstaja povezava. Razširitvi enačbe stožnice iz $\R^2$ v $P(\R^3)$ rečemo \emph{homogenizacija} -- v polinomu $f$, s katerim je podana stožnica v $\R^2$, vsak člen pomnožimo s tako potenco spremenljivke $z$, da je skupna stopnja vsakega člena enaka $2$, ter tako dobimo polinom $F$, s katerim je definirana stožnica v $P(\R^3)$. Obratnemu procesu, ko v polinomu $F$, ki definira stožnico v $P(\R^3)$, upoštevamo $z=1$ in tako dobimo polinom, ki poda stožnico v $\R^3$, pa rečemo \emph{dehomogenizacija}.
\end{opomba}

Naj sedaj stožnici $\mathcal{S}$ v $P(\R^3)$ pripada polinom $F$ z enačbo~\eqref{eq:stoznica_splosna}, kar lahko krajše zapišemo v obliki
\begin{equation}
...
Pri tem je realna simetrična matrika $M$ definirana do neničelnega skalarnega večkratnika natančno. Stožnica $\mathcal{S}$ je \emph{neizrojena} (ni unija dveh premic ali ene, dvojno štete premice), če ima poln rang, kar je ekvivalentno $\det M \neq 0$. V nadaljevanju bomo delali le z neizrojenimi stožnicami, torej vedno obstaja inverz matrike $M$, ki je prav tako simetrična matrika.

Če stožnico $\mathcal{S}$ s pripadajočo matriko $M$ zožimo na afini del $\Pi: z=1$, lahko iz prvega minorja matrike $M$ takoj preberemo, za katero vrsto stožnice gre (za algebraično izpeljavo gl.\ \cite{sivic2020}). Naj bo $A_M = a_{11}a_{22} - a_{12}^2$ prvi minor matrike $M$. Potem velja:
\begin{itemize}
    \item $\mathcal{S} \cap \Pi$ je hiperbola, če in samo če $A_M < 0$,
    \item $\mathcal{S} \cap \Pi$ je parabola, če in samo če $A_M = 0$ in
    \item $\mathcal{S} \cap \Pi$ je elipsa, če in samo če $A_M > 0$. Če poleg tega velja še $a_{11} = a_{22}$ in $a_{12} = 0$, je $\mathcal{S} \cap \Pi$ krožnica.
\end{itemize}
Ekvivalentno bi lahko zožitev stožnice na afini del določili z njenim presekom s premico v neskončnosti $p_\infty$, ki je v $\R^3$ ravnina $z = 0$:
\begin{itemize}
    \item če $\mathcal{S} \cap p_\infty = \emptyset$, je zožitev elipsa,
    \item če $\mathcal{S} \cap p_\infty = \{T\}$, je zožitev parabola in
    \item če $\mathcal{S} \cap p_\infty = \{T_1, T_2\}$, je zožitev hiperbola.
\end{itemize}

\begin{definicija}
    \emph{Tangenta} na stožnico $\mathcal{S} \subseteq P(\R^3)$ v točki $V \in \mathcal{S}$ je projektivna premica $p_V$, ki $\mathcal{S}$ seka le v projektivni točki $V$.
\end{definicija}
Naj bo $V = [x_0 : y_0 : z_0] \in \mathcal{S}$ in $v = (x_0, y_0, z_0)$. Potem velja $v^\intercal M v = 0$. Torej če za smerni vektor evklidske premice, ki jo predstavlja projektivna točka $V$, vzamemo $v$, je $Mv$ normala ravnine skozi koordinatno izhodišče, ki vsebuje to premico. Različni projektivni točki na stožnici $\mathcal{S}$ zaradi homogenosti koordinat določata normali z različnima smerema, zato ta ravnina vsebuje le točko $V$, torej je res tangenta. V $P(\R^3)$ je predstavljena s predpisom 
$$p_V =\{ [x : y : z] \in P(\R^3) \; | \; (x, y, z) M v = 0; v \in V \}.$$
\begin{definicija}
    Naj bo $\mathcal{S} \subset P(\R^3)$ stožnica v realni projektivni ravnini. Množico dualov tangent na stožnico $\mathcal{S}$ označimo s $\mathcal{S}^*$ in jo imenujemo \emph{dualna stožnica}.
\end{definicija}
\begin{trditev}
    Množica $\mathcal{S}^*$ je stožnica s pripadajočo matriko $M^{-1}$.
\end{trditev}
\begin{dokaz}
    Iz prejšnjega razdelka se spomnimo dualne preslikave. Vemo, da projektivni točki priredi tisto projektivno premico, ki ima kot evklidska ravnina za normalo smerni vektor premice, ki jo predstavlja projektivna točka, in da je to bijekcija. Tako je normala ravnine, ki predstavlja tangento $p_V$ na stožnico $\mathcal{S}$ v točki $V$, smerni vektor ortogonalnega komplementa od $p_V$ (slika~\ref{fig:proj_tangenta}).

    \begin{figure}[h]
        \centering
        \includegraphics[width=0.3\textwidth]{images/proj_tangenta.png}
        \caption[Projektivna tangenta]{Točka $V \in \mathcal{S}$ porodi tangento $p_V$ z dualom $q = Mv$.}
        \label{fig:proj_tangenta}
    \end{figure}

    Ker vsaka projektivna točka določa svojo tangento in se različne tangente slikajo v različne ortogonalni komplemente (ki so projektivne točke), je preslikava med $\mathcal{S}$ in $\mathcal{S}^*$ s predpisom $V \mapsto Mv$, kjer je $v \in V$, bijekcija. Naj bo $q = Mv$ neka točka na $\mathcal{S}^*$. Ker velja
    $$ q^\intercal M^{-1} q = (Mv)^\intercal M^{-1} (Mv) = v^\intercal M^\intercal M^{-1} M v = v^\intercal M v = 0,$$
    je $\mathcal{S}^*$ res stožnica, ki ji pripada matrika $M^{-1}$.
\end{dokaz}

% Pokažimo, da je $\mathcal{S}^*$ res stožnica. Če stožnici $\mathcal{S}$ pripadata polinom $F$ in posledično matrika $M$, je tangenta na točko $p \in \mathcal{S}$ projektivna premica $\{[x : y : z] \; | \; [x, y, z] M p = 0 \}$. Bralec se lahko s kratkim izračunom prepriča, da velja $[F_x(p), \; F_y(p), \; F_z(p)]^\intercal = 2 M \cdot p$, torej velja kar
...
% Če je $p \in \mathcal{S}$ in $ q \in \mathcal{S}^*$ tangenta na točko $p$, potem (z upoštevanjem homogenosti koordinat točk v projektivni ravnini) velja $q = M \cdot p$. Ker je matrika $M$ obrnljiva, to med drugim pomeni, da je preslikava $\mathcal{S} \longrightarrow \mathcal{S}^*$ s predpisom $p \mapsto Mp$ je bijekcija med $\mathcal{S}$ in $\mathcal{S}^*$. 

\begin{opomba}
    Ker velja $\left(M^{-1}\right)^{-1} = M$, je dualna stožnica dualne stožnice originalna stožnica.
\end{opomba}

Poglejmo, kako nam znanje o stožnicah in njihovih dualnih stožnicah pomaga pri reševanju kubičnih in kvartičnih enačb. Naj bosta $\mathcal{S}_1$ in $\mathcal{S}_2$ stožnici s pripadajočima simetričnima obrnljivima matrikama $M_1$ in $M_2$. Potem je dual skupne tangente skupna točka njunih dualnih stožnic $\mathcal{S}^*_1$ in $\mathcal{S}^*_2$. Torej iščemo $q \in \mathcal{S}^*_1 \cap \mathcal{S}^*_2$, ki reši sistem enačb
\begin{equation}
...
Naj bo kubična enačba oblike $ x^3 + bx^2 + cx + d = 0, d \neq 0$, in naj bosta paraboli $\mathcal{P}_1$ in $\mathcal{P}_2$ definirani takole:
\begin{itemize}
...
Enačbi parabol homogeniziramo in iz njiju zapišemo njuni matriki
$$ M_1 =
    \begin{bmatrix}
        0 & 0 & 2d\\
        0 & 1 & c\\
...
Naj bo $q = [A : B : C] \in \mathcal{P^*}_1 \cap \mathcal{P^*}_2$ dual skupne tangente na stožnici $\mathcal{P}_1$ in $\mathcal{P}_2$. Iz sistema enačb~\eqref{eq:afin_sistem_tangenta_splosen} dobimo nov sistem
\begin{equation}
    \label{eq:afin_sistem_tangenta_ABC_kub}
    bA^2 - cAB + AC + dB^2 = 0 \; \text{ in } \; -BC = A^2.
\end{equation}

Da dobimo tangento na paraboli $\mathcal{P}_1$ in $\mathcal{P}_2$ v afini ravnini $z = 1$, brez škode za splošnost (ker so koordinate projektivnih točk homogene) predpostavimo $B = -1$ in upoštevamo $z = 1$. S tem tangento $q: Ax + By + Cz = 0$ dehomogeniziramo v premico $y = Ax + C$ v afini ravnini. Druga enačba v sistemu~\eqref{eq:afin_sistem_tangenta_ABC_kub} se tako preuredi v $C = A^2$ in ko to vstavimo v prvo, dobimo enačbo
$$ A^3 + bA^2 + cA + d = 0,$$
kar pomeni, da nam $A$, koeficient skupne tangente, reši izvorno kubično enačbo.

\opomba{Opazimo, da je ideja postopka identična kot pri Geretschlägerjevi metodi (opomba~\ref{opom:geret_metoda}), le da so tu spremenljivke v enačbah parabol pomnožene s faktorjem $-2$. To je torej alternativen dokaz velavnosti metode preko projektivne geometrije.}

\textbf{Primer:} Rešimo kubično enačbo $x^3 - 2x^2 - x + 2 = 0$ po zgornjem postopku. Vzamemo paraboli
\begin{itemize}
    \item $\mathcal{P}_1: (y-1)^2 = -8(x+2)$ z goriščem $F_1 (-4, 1)$ in premico vodnico $L_1: x = 0$,
    \item $\mathcal{P}_2: x^2 = -4y$ z goriščem $F_2 (0, -1)$ in premico vodnico $L_2: y = 1$.
\end{itemize}
V koordinatni sistem zarišemo njuni gorišči in premici vodnici. Z Belochinim prepogibom konstruiramo tri skupne tangente (slika~\ref{fig:kubicna_tang_primer}) in razberemo njihove koeficiente. Če smo pri prepogibanju natančni, smo res dobili rešitve enačbe $2, 1$ in $-1$.
...
Avtorja nato predlagata naslednji matriki za dualni stožnici $\mathcal{C^*}$ in $\mathcal{P^*}$:
$$ M^{-1}_\mathcal{C} =
    \begin{bmatrix}
        1 & 0 & 0\\
        0 & 1 & 0\\
...
Iz enačbe parabole $\mathcal{P}: c^2x^2 - 2cdexy + d^2e^2y^2 - 4d^2ex = 0$ ne moremo enostavno prebrati gorišča in premice vodnice, zato $(x, y)$-sistem prevedimo v $(z, w)$-sistem s sledečo preslikavo:
...
\end{bmatrix}.
$$
Ko to vstavimo v matrično enačbo parabole v afini ravnini
$$\mathcal{P} : [x\;y\;1] \cdot P \cdot [x\;y\;1]^\intercal = [z\;w\;1] \cdot P \cdot [z\;w\;1]^\intercal = 0,$$
po računanju in poenostavljanju na koncu dobimo
...
$$ F_{xy} = P \cdot \begin{bmatrix} 0\\d^3e^2\\1\end{bmatrix} \longmapsto F_{xy} = O_{xy} + d^3e^2(de, c) = \left(\frac{c^2e}{b^2} + d^4e^3, -\frac{bc + cde^2}{b^2} + cd^3e^2\right),$$
$$ L_{xy} = P \cdot \left( \begin{bmatrix} 0\\-d^3e^2\\1\end{bmatrix} + t \cdot \begin{bmatrix} 1\\0\\1\end{bmatrix} \right) \longmapsto L_{xy}: (x, y) = O_{xy} - d^3e^2(de, c) + t(c, -de), t \in R.$$
...
\opomba{Faucette v svojem članku opisuje geometrijsko interpretacijo reševanja \emph{splošnih} kvartičnih enačb, iz katerih določi stožnici in išče njuna presečišča v projektivni ravnini. Za več gl.\ \cite{faucette1996}.}