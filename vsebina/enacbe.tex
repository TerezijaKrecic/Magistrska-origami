...
\begin{definicija}
    ...
    \end{equation}
    za $a_i \in \R$. Stožnica v realni projektivni ravnini $P_2(\R)$ je množica $\mathcal{S} \subset P_2(\R)$, ki vsebuje natanko ničle tega polinoma, tj.
...
\begin{definicija}
    \emph{Dualna stožnica} $\mathcal{\hat{S}}$ za dano stožnico $\mathcal{S}$ v projektivni ravnini $P_2(\C)$ je množica tangentnih premic na $\mathcal{S}$. Če stožnici $\mathcal{S}$ pripadata polinom $F(x,y,z)$ in posledično matrika $M$, je tangenta na točko $v \in \mathcal{S}$ dana z enačbo $x F_x(v) + y F_y(v) + z F_z(v) = 0$. Torej je
    $$ \mathcal{\hat{S}} = \{[F_x(v) : F_y(v) : F_z(v)]; v \in \mathcal{S} \}.$$
\end{definicija}
Naj bo $v \in \mathcal{S}$ in $ q \in \mathcal{\hat{S}}$ tangenta na točko $v$. Bralec se z upoštevanjem homogenosti koordinat točk v projektivni ravnini lahko hitro prepriča, da potem velja $q = M \cdot p$. To med drugim pomeni, da je preslikava $\mathcal{S} \longrightarrow \mathcal{\hat{S}}$ s predpisom $p \mapsto Mp$ je bijekcija med $\mathcal{S}$ in $\mathcal{\hat{S}}$.
...
Po Bezoutovem izreku \textcolor{red}{(referiraj na izrek)} imata dve različni in neizrojeni stožnici v kompleksni projektivni ravnini štiri presečišča vključno z večkratnostjo. Ker sta njuni dualni stožnici prav tako v kompleksni projektivni ravnini, to pomeni, da imata stožnici v kompleksni projektivni ravnini štiri skupne tangente, ki nastopajo v konjugiranih parih -- po nič, dve ali štiri tangente (šteto z večkratnostjo). Dve paraboli imata skupno tangento v neskončnosti, torej ostanejo le še tri afine tangente, kar ni dovolj za vse rešitve kvartične enačbe. Zato tu ne moremo vzeti dveh parabol.
