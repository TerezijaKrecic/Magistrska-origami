...
Realna projektivna ravnina $P(\R^3)$ sestoji iz množice točk in premic:
\begin{itemize}
    \item točke v $P(\R^3)$ ustrezajo premicam skozi izhodišče v evklidskem prostoru $\R^3$,
    \item premice v $P(\R^3)$ ustrezajo ravninam skozi izhodišče v evklidskem prostoru $\R^3$.
\end{itemize}
V projektivni ravnini se sekata tudi vzporedni premici, in sicer v \emph{točki v neskončnosti}. Tako so točke in premice tu v incidenčni relaciji in veljajo aksiomi projektivne ravnine:
\begin{itemize}
...
\begin{definicija}
    Naj bodo $a_{11}, a_{12}, a_{22}, a_{13}, a_{23}, a_{33} \in \R$. Stožnica v realni ravnini $\R^2$ je množica točk $(x, y)$, ki rešijo enačbo
    $$a_{11}x^2 + 2a_{12}xy + a_{22}y^2 + 2a_{13}x + 2a_{23}y + a_{33} = 0.$$
    Stožnica v realni projektivni ravnini $P(\R^3)$ je množica točk $(x, y, z)$, ki rešijo enačbo
    \begin{equation}
        \label{eq:stoznica_splosna}
        a_{11}x^2 + 2a_{12}xy + a_{22}y^2 + 2a_{13}xz + 2a_{23}yz + a_{33}z^2 = 0,
    \end{equation}
\end{definicija}
\begin{opomba}
    Med enačbama stožnic v obeh ravninah obstaja povezava. Razširitvi enačbe stožnice v $\R^2$ v $P(\R^3)$ rečemo \emph{homogenizacija}, obratnemu procesu (zožitvi stožnice na afini del, ravnino $z = 1$) pa \emph{dehomogenizacija}.
\end{opomba}

Naj ima sedaj stožnica $\mathcal{S}$ ima v $P(\R^3)$ homogenizirano enačbo~\ref{eq:stoznica_splosna}, kar lahko krajše zapišemo v obliki
\begin{equation*}
...
Pri tem je simetrična matrika $M$ definirana do neničelnega skalarnega večkratnika natančno. Stožnica $\mathcal{S}$ je \emph{neizrojena} (ni unija dveh premic ali ene, dvojno štete premice), če ima poln rang, torej rang $ M = 3$ oziroma ekvivalentno $\det M \neq 0$. V nadaljevanju bomo delali le z neizrojenimi stožnicami, torej vedno obstaja inverz matrike $M$, ki je prav tako simetričen.
...
\begin{definicija}
    \emph{Dualna krivulja} za dano krivuljo $\mathcal{C}$ v projektivni ravnini je krivulja v dualni projektivni ravnini, ki jo sestavlja množica tangentnih premic na $\mathcal{C}$.
\end{definicija}

Naj bo $\mathcal{S}$ stožnica s pripadajočo matriko $M$. Definirajmo stožnico $\mathcal{\hat{S}}$ s predpisom
\begin{equation*}
    \mathcal{\hat{S}}: v^\intercal M^{-1} v = 0.
\end{equation*}
Naj bo $p \in \mathcal{S}$ točka na stožnici $\mathcal{S}$ in naj bo $q = Mp$. Potem je
$$ q^\intercal p = q^\intercal M^{-1} q = (Mp)^\intercal M^{-1} (Mp) = p^\intercal M^\intercal M^{-1} M p = p^\intercal M p = 0,$$
kar pomeni dvoje:
\begin{itemize}
    \item $q \in \mathcal{\hat{S}}$ in preslikava $\mathcal{S} \longrightarrow \mathcal{\hat{S}}$ s predpisom $p \mapsto Mp$ je bijekcija med stožnicama $\mathcal{S}$ in $\mathcal{\hat{S}}$,
    \item $q$ je tangenta na $\mathcal{S}$ v točki $p$ \textcolor{red}{(Je to ok formulirano?)}.
\end{itemize}
S tem smo izpeljali, da je stožnica $\mathcal{\hat{S}}$ s pripadajočo metriko $M^{-1}$ dualna stožnica za $\mathcal{S}$.
...
Naj bo $q = [A\;B\; C]^\intercal \in \mathcal{\hat{P}}_1 \cap \mathcal{\hat{P}}_2$ skupna tangenta na paraboli $\mathcal{P}_1$ in $\mathcal{P}_2$. Iz sistema enačb~\ref{eq:afin_sistem_tangenta_splosen} dobimo nov sistem
\begin{equation}
...
\end{itemize}
Bralec je povabljen, da po enakem postopku kot pri kubični enačbi izpelje, da koeficient skupne tangente $y = Ax + C$ reši enačbo~\ref{eq:reduc_kvart_ev}. Stožnicama $\mathcal{S}$ in $\mathcal{P}$ zaporedoma pripadata matriki
$$ M_\mathcal{S} =
    \begin{bmatrix}
        d & c & 0\\
        c & b & 0\\
...
V dobljeno enačbo vstavimo enakosti~\ref{eq:kvarticni_relaciji} in dobimo
$$C^4 + bC^2 + 2cC + d = 0,$$
...
Začetne vrednosti tangent so res v njenih rešitvah, torej v točkah $1, 5/8, 3/8$ in $-2$.