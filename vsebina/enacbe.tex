...
% Predstavljajmo si, da realno ravnino $\R^2$ postavimo na nivo $z = 1$.

% Če za projektivno točko $[x_0 : y_0 : z_0]$ velja $z_0 \neq 0$, lahko njene koordinate zaradi homogenosti delimo z $z_0$ in dobimo isto projektivno točko $[x'_0 : y'_0 : 1]$, kjer velja $x'_0 = x_0/z_0$ in $y'_0 = y_0/z_0$. Torej ta projektivna točka določa točko $(x'_0, y'_0, 1)$ v $\R^3$, ki leži na ravnini $z=1$. 
...
Evklidske premice skozi koordinatno izhodišče, ki ne sekajo ravnine $z = 1$ (torej ležijo na ravnini $z=0$), predstavljajo točke, ki jih imenujemo \emph{točke v neskončnosti}. Te projektivne točke imajo tretjo koordinato ničelno. Sama ravnina $z = 0$ predstavlja t.~i.\ \emph{premico v neskončnosti}.
...
\textcolor{red}{Opcija: v tem razdelku ne omenim dualnosti (potem tudi izbriši ``in dualnost'' in podnaslova). V tem primeru moraš definirati, kaj je dualna projektivna ravnina, pri definiciji dualne stožnice \ldots}
...
Vse neizrojene stožnice v projektivni ravnini $P(\R^3)$
%so med seboj ekvivalentne in
imajo v $\R^3$ obliko stožca. Naj bo stožnica $\mathcal{S}$ s pripradajočo matriko $M$ neizrojena. Če jo zožimo na afini del $\Pi: z=1$, lahko iz prvega minorja matrike $M$ takoj preberemo, za katero vrsto neizrojene stožnice gre (za algebraično izpeljavo gl.\ \cite{sivic2020}). Naj bo $A_M = a_{11}a_{22} - a_{12}^2$ prvi minor matrike $M$. Potem velja:
\begin{itemize}
