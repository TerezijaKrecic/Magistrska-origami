...
Naj sedaj stožnici $\mathcal{S}$ v $P(\R^3)$ pripada homogenizirana enačba~\eqref{eq:stoznica_splosna}, kar lahko krajše zapišemo v obliki
\begin{equation}
...
Pri tem je realna simetrična matrika $M$ definirana do neničelnega skalarnega večkratnika natančno. Stožnica $\mathcal{S}$ je \emph{neizrojena} (ni unija dveh premic ali ene, dvojno štete premice), če ima poln rang, torej rang $ M = 3$, kar je ekvivalentno $\det M \neq 0$. V nadaljevanju bomo delali le z neizrojenimi stožnicami, torej vedno obstaja inverz matrike $M$, ki je prav tako simetričen.

Naj bo stožnica $\mathcal{S}$ s pripradajočo matriko $M$ neizrojena. Če jo zožimo na afini del $\Pi: z=1$, lahko iz prvega minorja matrike $M$ takoj preberemo, za katero vrsto neizrojene stožnice gre (za algebraično izpeljavo gl.\ \cite{sivic2020}). Naj bo $A_M = a_{11}a_{22} - a_{12}^2$ prvi minor matrike $M$. Potem velja:
\begin{itemize}
...
\begin{definicija}
    \emph{Tangenta} na stožnico $\mathcal{S} \subseteq P(\R^3)$ v točki $A \in \mathcal{S}$ je premica, ki $\mathcal{S}$ seka le v točki $A$.
\end{definicija}
\begin{opomba}
    V projektivni ravnini je to ravno \emph{polara} projektivne točke $A \in \mathcal{S}$, tj.\ projektivna premica
    $$p_A =\{ [x : y : z] \in P(\R^3) \; | \; (x, y, z) M a = 0; a \in A \}.$$
    V $\R^3$ je to ravnina skozi koordinatno izhodišče z normalo $\vec{n} = M \cdot \vec{a}$.
\end{opomba}
\begin{definicija}
    Naj bo $\mathcal{S} \subset P(\R^3)$ stožnica v realni projektivni ravnini. Njen dual imenujemo \emph{dualna stožnica} in je množica $\mathcal{S}^* \subset P^*(\R^3)$ v dualni projektivni ravnini, katere elementi so tangentne premice na stožnico $\mathcal{S}$, tj.\
...
\begin{opomba}
    Ker velja $\left(M^{-1}\right)^{-1} = M$, je dualna stožnica dualne stožnice res originalna stožnica, kar ustreza principu dualnosti.
\end{opomba}
