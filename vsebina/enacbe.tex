...
Spoznali bomo dva postopka, ki nam z uporabo Belochinega pregiba rešita kubično enačbo. Najprej bomo pogledali bolj znano metodo, ki jo je odkrila Belocheva sama, nato pa še en podoben, a tudi enostavnejši postopek. Pri obeh postopkih nam Belochin pregib poda eno od rešitev kubične enačbe. Vsaka izmed rešitev kubične enačbe nam da drugačen pregib in ker so rešitve največ tri, so tudi pregibi oz.\ skupne tangente na dve paraboli največ tri. S postopkoma tako v resnici dokažemo naslednjo trditev.

\begin{trditev}
    Število realnih rešitev kubične enačbe je enako številu Belochinih pregibov v postopkih, ki sta navedena v tem poglavju.
\end{trditev}

\textcolor{red}{če ima alperin (zadnji razdelek na koncu tega poglavja) še kakšen postopek, popravi to trditev.}

\subsubsection{Reševanje enačbe preko Lillove metode}

Belocheva je s svojim pregibom oblikovala postopek, s katerim lahko preko origamija rešujemo kubične enačbe. Postopek temelji na Lillovi genialni metodi iskanja ničel poljubnih polinomov z realnimi koeficienti, ki si jo bomo v naslednjem razdelku podrobneje pogledali, za njeno aplikacijo pa uporabi avtorsko konstrukcijo -- Belochin kvadrat. \textcolor{red}{zaenkrat je tukaj še splošna kubična enačba, ampak a je mogoče treba predpostavit, da noben koeficient ni ničeln?}
...
\subsubsection{Reševanje enačbe preko koeficienta tangente na paraboli}

Poglejmo še en način reševanja.



Koeficient $t$ ene skupne tangente na parabolo je torej rešitev kubične enačbe $x^3 + b x^2 + c x + d = 0$, ki pa ima največ tri realne rešitve. Vsaka izmed njih je koeficient neke tangente na parabolo, vemo pa že, da je tangenta z danim koeficientom in parabolo natanko določena \textcolor{red}{(preveri, a je ta odstavek res red)}. Torej lahko reševanje kubičnih enačb enačimo z iskanjem skupnih tangent na dve paraboli.
