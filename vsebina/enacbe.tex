...
Poljubna ravnina v $\R^3$ skozi koordinatno izhodišče ima enačbo $ax+by+cz=0$, kjer so $a, b, c \in \R$, ne vsi enaki $0$. Projektivna premica, ki jo ta ravnina določa, je tako množica projektivnih točk $\{ [x:y:z]; ax+by+cz = 0\}$.
...
Vse neizrojene stožnice v projektivni ravnini $P(\R^3)$ so med seboj ekvivalentne in imajo obliko stožca v $\R^3$. Naj bo stožnica $\mathcal{S}$ s pripradajočo matriko $M$ neizrojena. Če jo zožimo na afini del $\Pi: z=1$, lahko iz prvega minorja matrike $M$ takoj preberemo, za katero vrsto neizrojene stožnice gre (za algebraično razlago gl.\ \cite{sivic2020}). Naj bo $A_M = a_{11}a_{22} - a_{12}^2$. Potem velja:
\begin{itemize}
...
\begin{definicija}
    \emph{Tangenta} na stožnico $\mathcal{S}$ v točki $A \in \mathcal{S}$ je premica, ki $\mathcal{S}$ seka le v točki $A$.
\end{definicija}
\begin{opomba}
    V projektivni ravnini je to ravno \emph{polara } projektivne točke $A$ glede na $\mathcal{S}$, tj.\ projektivna premica
...
Če stožnici $\mathcal{S}$ pripadata polinom $F$ in posledično matrika $M$, je tangenta na točko $p \in \mathcal{S}$ projektivna premica $\{[x : y : z] \; | \; [x \; y \; z] M p = 0 \}$. Bralec se lahko s kratkim izračunom prepriča, da velja $[F_x(p), \; F_y(p), \; F_z(p)]^\intercal = 2 M \cdot p$, torej velja kar
