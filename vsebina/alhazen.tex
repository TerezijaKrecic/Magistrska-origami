...
Grški matematik Heron iz Aleksandrije je okoli leta 100 po Kr.\ zastavil in rešil naslednje vprašanje, ki je različica Ptolemajevega problema: \emph{Na isti strani premice ležita točki $A$ in $B$. Poišči točko $C$ na premici, da bo pot od točke $A$ do $B$ preko točke $C$ najkrajša} (predpostavimo evklidsko metriko).
...
Enačbo kvadriramo, spet upoštevamo pogoj~\eqref{eq:pogoj_alh1} in dobimo enačbo četrte stopnje:
...
\begin{dokaz}
    ...
    Naj bo $S$ presečišče premice $CO$ z daljico $A'B$. Ker točki $C$ in $A$ ležita na krožnici $\mathcal{K}$, je trikotnik $\triangle CAO$ z vrhom v središču $O$ enakokrak. Sledi $\angle OCA = \angle CAO = \angle CA'D$. Zadnja enakost sledi iz sovršnih kotov ob točki $A$ in simetrije čez pregib $p$. Torej sta kota z vrhom v točkah $C$ in $A'$ (na sliki~\ref{fig:scimemi_dokaz} desno) izmenična, iz česar sledi $CS \parallel DA'$. Zato velja $\angle OSB = \angle DA'B = 90^\circ$.
...
Nishimura v~\cite{nishimura2018} povzema, kako je Huygens preko kompleksnih števil Alhazenov problem prevedel na iskanje presečišča krožnice z določeno hiperbolo, nekaj več razlage pri izpeljavi pa opiše tudi Alperin v~\cite{alperin2002}. Nishimura od tu nadaljuje v smer, ki nam sproducira konstrukcijo rešitve z origamijem, in po prevedbi problema na iskanje skupne tangente na dve stožnici avtor svoj članek zaključi še z navedbo konkretne origami konstrukcije, ki konstruira rešitev Alhazenovega problema.

V nadaljevanju privzamemo avtorjeve oznake objektov. Brez škode za splošnost naj bo $\mathcal{C}$ enotska krožnica s središčem $O$. Znotraj nje izberemo poljubni točki $A$ in $B$ taki, da so točke $A,B,O$ paroma različne (slika~\ref{fig:huygens1}). Išemo točko $P \in \mathcal{C}$, da bo veljalo
\begin{equation}
...
Če delimo kompleksni števili z enakima argumentoma, se njuna kota odštejeta v $0$, torej dobimo pozitivno realno število. Njegovo konjugirano število mu je torej enako. Zato velja enačba
\begin{equation*}
...
\end{equation*}
Ker sta koeficienta ob kvadratnih členih različno predznačena, je to enačba hiperbole. Torej je točka $P$ presečišče krožnice~$\mathcal{C}$ in te hiperbole (slika~\ref{fig:huygens2}).
%Hungerbühler v~\cite{hungerbuhler1992} dobljeno hiperbolo bolj podrobno določi, vendar to za nas ni ključno. 
...
Bralcu za vajo prepuščamo računanje dualnih stožnic $\mathcal{C^*}$ in $\mathcal{Q^*}$ (gl.\ klasifikacijo stožnic z matriko v razdelku~\ref{podpogl:stoznice_projektivna}). Dobimo
$$ \mathcal{C^*}: x^2 + y^2 - 1 = 0 \; \text{ in } \; \mathcal{Q^*}: r^2x^2 + 2rsxy + s^2y^2 + 4qrx - 4qsy + 4q^2 = 0. $$

\begin{opomba}
    Za izračun dualne stožnice $\mathcal{Q^*}$ je potrebna predpostavka $qrs \neq 0$. Če pa je katero od števil $q, r, s$ ničelno, je hiperbola izrojena, torej se nam problem zreducira na iskanje skupnih presečišč krožnice in dveh premic, kar je računsko zelo enostavno in nas zato ne zanima.
\end{opomba}

Dualna krožnica enotske krožnice je torej prav tako enotska krožnica, stožnica $\mathcal{Q^*}$ pa je parabola (ker je $r^2 \cdot s^2 - \frac{(2rs)^2}{4} = 0$, tj.\ glavni minor pripadajoče matrike je ničeln). Problem je sedaj preveden na iskanje skupne tangente na krožnico $\mathcal{C}$ in parabolo $\mathcal{Q^*}$. To nas razveseli, saj to z origamijem zmoremo storiti.

Kako pa bomo iz skupne tangente določili točko $P$? Iz originalnega iskanja skupne točke $P \in \mathcal{C} \cap \mathcal{Q}$ preidemo na iskanje skupne tangente $p$ na dualni stožnici $\mathcal{C^*}$ in $\mathcal{Q^*}$. Ker je $\mathcal{C} = \mathcal{C^*}$, je $p$ ravno tangenta na to krožnico v točki $P$. Iskana točka $P$ je torej dotikališče skupne tangente s krožnico $\mathcal{C}$. Zdaj moramo samo še določiti gorišče in premico vodnico parabole $\mathcal{Q^*}$.

\begin{trditev}
    \label{trd:alh_parab_enacba}
    Parabola $\mathcal{Q^*}$ z enačbo $r^2x^2 + 2rsxy + s^2y^2 + 4qrx - 4qsy + 4q^2 = 0$ ima gorišče $F = \left( \frac{-2qr}{r^2+s^2}, \frac{2qs}{r^2+s^2} \right)$ in premico vodnico $D: sx-ry = 0$.
\end{trditev}
\begin{dokaz}
    Naj bo $X = (x_0, y_0)$ točka, ki je enako oddaljena od gorišča $F$ in premice vodnice $D$. Dokazati moramo, da so njene koordinate rešitve enačbe za parabolo $\mathcal{Q^*}$.
    ...
    Ko izraze na obeh straneh poenostavimo in malo preuredimo enačbo, dobimo $r^2x_0^2 + 2rsx_0y_0 + s^2y_0^2 + 4qrx_0 - 4qsy_0 + 4q^2 = 0$, torej je $\mathcal{Q^*}$ res parabola.
\end{dokaz}

Tako smo prišli do konca teoretičnega postopka. Na hitro ponovimo -- pri dani enotski krožnici $\mathcal{C}$ in točkama $A, B$ znotraj nje (postopek velja tudi za točki zunaj krožnice) nam presečišče $P$ krožnice $\mathcal{C}$ s hiperbolo $\mathcal{Q}$, ki je odvisna od točk $A$ in $B$, reši Alhazenov problem. Presečišče stožnic prevedemo v iskanje skupne tangente $p$ na krožnico $\mathcal{C}$ in parabolo $\mathcal{Q^*}$, ki je dualna stožnica hiperbole (spomnimo pa se, da tangento dobimo s hkratnim prepogibom središča krožnice na rob dvakrat večje krožnice in gorišča parabole na njeno premico vodnico). Iskana točka $P$ je dotikališče tangente $p$ s krožnico $\mathcal{C}$ (slika~\ref{fig:nishimura_resitev}).
...
\begin{enumerate}
    ...
    \item Skozi točki $O$ in $M$ prepognemo premico $D$ (ker je $ M = (a+b)/2$, je to ravno premica vodnica parabole $\mathcal{Q^*}$). Presečišče poltraka $OM$ s krožnico $\mathcal{C}$ označimo z $E'$.
    ...
    \item Prepognemo premico $OB$ in njeno presečišče s premico $l_2$ označimo z $G$. Iz razmerja podobnih trikotnikov $\triangle AOG$ in $\triangle EOB$ sledi $|OG| = q = \mathrm{Re}(ab)$ (slika~\ref{fig:nishimura_origami}).
    \begin{figure}[h]
        \centering
    ...
    \begin{figure}[h]
        ...
        \caption[Nishimurijeva konstrukcija $2$]{Nishimurijeva konstrukcija gorišča $F$ (korak $13$). Vzeto iz~\cite[str.\ 42]{nishimura2018}.}
        \label{fig:nishimura_origami2}
    \end{figure}
    
    Ker je premica $OF$ zrcalna slika premice $D$ čez abscisno os, je njena enačba $OF: sx + ry = 0$. Zaradi simetrije velja $|OF| = |OK| = |OH| = 2q/ \sqrt{s^2+r^2}$. Za točko $F = (x,y)$ tako dobimo sistem enačb
    $$ sx+ry=0 \; \text{ in } \; \sqrt{x^2+y^2} = \frac{2q}{\sqrt{s^2+r^2}},$$
    katerega rešitev so ravno koordinate gorišča iskane parabole iz trditve~\ref{trd:alh_parab_enacba} (lahek izračun je prepuščen bralcu), torej je točka $F$ gorišče parabole $\mathcal{Q^*}$ s premico vodnico $D$.
    
    \item Za določitev iskane točke $P$ samo še prepognemo tangento $p$ na krožnico $\mathcal{C}$, ki točko $F$ položi na premico $D$. Točka $P$ je dotikališče tangente $p$ s krožnico $\mathcal{C}$ (slika~\ref{fig:nishimura_resitev}).
\end{enumerate}
