...
\textcolor{red}{opiši konstrukcijo iz~\cite[str.\ 28]{hull2020}.}

Ker lahko čez premico zrcalimo točke, lahko čeznjo zrcalimo tudi daljice oz.\ premice (zrcalimo dve točki z daljice in naredimo pregib čez njuni sliki). Zrcaljenje pa nam omogoča še nekaj več, kar nam pove naslednja trditev.

% moja avtorska trditev in dokaz:)

\begin{trditev}
    \label{trd:prenasanje_razdalj}
    S prepogibanjem papirja z upoštevanjem pravil iz~\ref{origami_konstrukcije} in origami operacij lahko prenašamo razdalje.
\end{trditev}

\begin{dokaz}
    V ravnini si izberimo poljubni točki $A$ in $B$, ki določata daljico z neko dolžino $r$. Naj bo $A'$ poljubna točka in $a$ poljubna premica skoznjo. Trditev pravi, da lahko z origami operacijami konstruiramo točko $B' \in a$, da je $d(A'B') =r$ (slika~\ref{fig:prenos_razdalje_konec}). Pri tem ni pomembno, na kateri strani točke $A'$ leži točka $B'$, saj jo lahko vedno zrcalimo čeznjo.
    ...
    Prenos razdalje razdelimo na dva dela:
    \begin{enumerate}
        ...
        \item Daljico $A'B''$ rotiramo okoli točke $A'$, da se točka $B''$ preslika na premico $a$. To storimo preko simetrale kota med daljico $A'B''$ in premico $a$ (operacija~\ref{op:O4}) in konstrukcije pravokotnice na to simetralo oz. zrcaljenje točke $B''$ čeznjo (slika~\ref{fig:prenos_razdalje_koraki} desno). S tem dobimo točko $B'$, ki je po konstrukciji od točke $A'$ oddaljena za dolžino $r$.
    \end{enumerate}
...
Da nam prepogibanje papirja omogoča prenašanje razdalj, bomo uporabili v podpoglavju~\ref{origami_konstruktibilnost}.
...
\begin{izrek}
    Za poljubne $a, b, r \in \Q$, pri čemer $r > 0$ in $\sqrt{r} \notin \Q$, so origami-konstruktibilna vsa števila iz množice $\Q(r) = \{a + b\sqrt{r}\}$.
\end{izrek}

\begin{dokaz}
    Dokazati moramo, da znamo s prepogibanjem seštevati, odštevati, množiti, deliti in koreniti dana oz.\ že konstruirana števila. Na začetku imamo po točki $(1, 0)$ na abscisni osi dano število 1. Brez težav lahko konstruiramo točko $(x, 0)$ za poljuben $x \in \Z$, saj znamo iz abscisne osi in dveh točk, danih v definiciji, konstruirati vertikalne premice in zrcaliti točko čeznje. Ker lahko z origamijem prenašamo razdalje, je tudi seštevanje in odštevanje že konstruiranih števil enostavno. Množenje je poseben primer seštevanja, deljenje pa \textcolor{red}{referenca na poglavje, kjer se pokaže delitev na $n$ delov, da se tam dokaže}. Tako lahko konstruiramo poljuben $x \in \Q$. Manjka nam samo še konstrukcija $\sqrt{x}$ za poljuben $x \in \Q$. V ta namen si poglejmo naslednjo konstrukcijo:

    \textcolor{red}{konstrukcija korena iz x na str. 58 Hull 2013 vnesi v bib}
\end{dokaz}
