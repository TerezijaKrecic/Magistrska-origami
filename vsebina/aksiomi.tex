...
\begin{izrek}
    Velja $\E = F_n = \Q(\sqrt{r_1}, \sqrt{r_2}, \ldots, \sqrt{r_n})$, kjer je $F_n$ neka iterativna kvadratna razširitev polja $\Q$.
\end{izrek}

Navedimo le idejo dokaza. Da je vsako število iz množice $F_n$ evklidsko konstruktibilno, smo lahko razmislili tik pred navedbo tega izreka. Da velja tudi $\E \subset F_n$, pa je potrebna obravnava vseh možnih enačb, ki jih dobimo pri iskanju presečišč dveh krožnic, dveh premic ter krožnice in premice. Bralec si lahko natančen dokaz ogleda v~\cite[str.\ 38--39]{geometricconstructions} ali v~\cite{jerman1998}.

\begin{posledica}
    \label{izr:evkl_konstr}
    Število $a \in \R$ se da konstruirati le s pomočjo ravnila in šestila natanko tedaj, ko obstaja iterativna kvadratna razširitev $F$ polja $\Q$, da je $a \in F$ in razsežnost polja $F$ enaka $2^n$ za nek $n \in \N_0$.
\end{posledica}

\textcolor{red}{daj kakšen primer, kako lahko zgleda število iz $\E$, npr.\ iz~\cite[str.\ 37]{geometricconstructions}. Pač vse, kar lahko v kalkulatorju narediš z unimi simboli, ki spodi pišejo.}
